...
def main(): 
    ...
    for radius, height, name, cost in cans: 
	    if compute_cost_efficiency(radius,height,cost)> best_cost_efficiency: 
                best_cost_efficiency = compute_cost_efficiency(radius,height,cost) 
    ...
    for radius, height, name, cost in cans: 
        if compute_storage_efficiency(radius,height) == best_storage_efficiency:
            print(f'{name}: {compute_storage_efficiency(radius, height):.2f} ' 
                ...
                f'best storage efficiency')
        elif  compute_cost_efficiency(radius,height,cost) == best_cost_efficiency:
            print(f'{name}: {compute_storage_efficiency(radius, height):.2f} ' 
                f'{compute_cost_efficiency(radius, height, cost):.2f} '
                f'best cost efficiency')
        else:
            print(f'{name}: {compute_storage_efficiency(radius, height):.2f} ' 
                f'{compute_cost_efficiency(radius, height, cost):.2f}')
